...
var buildHandler = require('./handlerTemplate');
...
/**
 * Processes an x-backend-setup config and puts a request templates
 * for each resource to the node value.
 *
 * @param {object} setupConf an endpoint configuration object
 * @param {object} node a current spec tree node to alter
 */
Router.prototype._handleSetupConf = function(setupConf, node) {
    if (Array.isArray(setupConf)) {
        setupConf.forEach(function(resourceSpec) {
            Object.keys(resourceSpec).forEach(function(requestName) {
                ...
                node.value.resources.push(requestSpec);
            });
        });
    } else {
        throw new Error('Invalid config. x-backend-setup must be an array');
    }
};
...
Router.prototype._handleSwaggerPathSpec = function(node, pathspec,
    ...
    .then(function() {
        ...
        Object.keys(pathspec).forEach(function(methodName) {
            ...
            var backendSetup = method && method['x-backend-setup'];
            if (backendSetup) {
                self._handleSetupConf(backendSetup, node);
            }

            var backendRequest = method && method['x-backend-request'];
            if (backendRequest) {
                node.value.methods[methodName] = buildHandler(backendRequest);
