        ...
        operations, specRoot, prefixPath)
{
    ...
    // Check if there is a new spec root
    var newSpecRoot = pathspec['x-spec-root'];
    if (newSpecRoot) {
        specRoot = newSpecRoot;
        specRoot.paths = {};
    ...
    .then(function() {
        ...
        if (specRoot && !specRoot.paths[prefixPath] && !/^\/sys\//.test(prefixPath)) {
...
Router.prototype._handleSwaggerSpec = function (rootNode, spec, operations, specRoot, prefixPath) {
    ...
    function handlePaths (paths) {
        ...
        return Promise.all(Object.keys(paths).map(function(pathPattern) {
            ...
            var subPrefixPath = prefixPath + pathURI.toString({
                asValuePattern: true
            });
