...
function makeRequestTemplate(spec) {
    var uri = new URI(spec.uri, {}, true);
    return function (restbase, req) {
        // FIXME:
        // - Better way to get a URI clone with parameters expanded, without
        //   going via a string.
        // - Template other request parts
        return restbase.request({
            uri: new URI(uri.bind(req.params)).toString(),
            method: req.method,
            body: req.body,
            headers: req.headers,
            query: req.query
        ...
        Object.keys(subspec).forEach(function(methodName) {
            ...
            if (mxrb && mxrb.service) {
                ...
                if (handler) {
                    ...
                    node.value.m = [node.value.m, methodName];
