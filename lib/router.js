 ...
 */
Router.prototype._loadModule = function(name, modDef, config) {
        ...
        case 'spec':
            if (modDef.path && /^\//.test(modDef.path)) {
                ...
                if (modDef.path) {
                    ...
                    loadPath += 'mods/' + (modDef.name || name);
                }
            }
            break;
        case 'npm':
            loadPath = modDef.name || name;
            break;
        default:
            throw new Error('unknown module type '
                + modDef.type + ' (for module ' + name + ').');
    }
    ...
    if (modDef.type === 'spec') {
        ...
        if (modObj instanceof Function) {
            modObj = modObj(options, modDef.templates);
        }
...
Router.prototype._loadModules = function(restBaseModules, node, specRoot, prefixPath) {
    var self = this;
    if (Array.isArray(restBaseModules)) {
        ...
        return P.each(Object.keys(restBaseModules), function(name) {
            var m = restBaseModules[name];
            // Share modules
            return self._loadModule(name, m, specRoot._config)
            .then(function(module) {
                if (!module) {
                    throw new Error('Null return when loading module ' + name);
                }
                if (!module.spec) {
                    throw new Error('Module ' + name + ' did not export a spec.');
                }
...
Router.prototype._handleSwaggerPathSpec = function(node, pathspec,
    ...
    if (prefixPath.split('/').length === 3) {
        ...
        // console.log(specRoot, prefixPath, pathspec);
        loaderPromise = loaderPromise.then(function() {
            return self._handleSwaggerSpec(node, pathspec,
                    {}, specRoot, prefixPath);
        });
    } else if (pathspec.swagger) {
        // We are loading a sub-spec.
        loaderPromise = self._handleSwaggerSpec(node, pathspec,
                operations, specRoot, prefixPath);
    }
    ...
    if (restBaseModules) {
        loaderPromise = loaderPromise.then(function() {
            return self._loadModules(restBaseModules, node, specRoot, prefixPath);
        });
    }
    ...
    .then(function() {
        ...
        Object.keys(pathspec).forEach(function(methodName) {
            ...
            if (specRoot.paths[prefixPath] && methodName
                    && !specRoot.paths[prefixPath][methodName]) {
    ...
    function handlePaths(paths) {
        ...
        return P.each(Object.keys(paths), function(pathPattern) {
            ...
            if (!subtree) {
                ...
                // console.log('cloned',
                //    JSON.stringify(origSubtree, null, 2),
...
Router.prototype.loadSpec = function(spec, restbase) {
    ...
    return specPromise
    .then(function(spec) {
        // Hack for now, as we don't have a clean options object that is
        // passed down the tree. FIXME!
        spec._config = restbase.rb_config;
