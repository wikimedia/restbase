...
/**
 * Creates a request post-processor from the spec.
 *
 * @param {Array} spec - a request post-processor spec
 * @returns {Function} a request post-processor function
 */
function createPostProcessor(spec) {
    var processorChain = spec.map(function(stepSpec) {
        stepSpec = stepSpec[Object.keys(stepSpec)[0]];
        return new Template({
            uri: stepSpec.request.uri,
            method: 'post',
            body: {
    ...
    return function(restbase, originalReq, originalRes, endpointSpec) {
        var promise = P.resolve(originalRes);
        processorChain.forEach(function(processorRequestTemplate) {
            promise = promise.then(function(response) {
                return restbase.request(processorRequestTemplate.expand({
                    request: originalReq,
                    response: response,
                    spec: endpointSpec
                }));
...
module.exports = {
    ...
    createPostProcessor: createPostProcessor
};
