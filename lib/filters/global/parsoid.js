...
function parsoidConvert(parsoidHost, from, to) {
    return function (restbase, req) {
        var domain = req.params.domain;
        var uri = parsoidHost + '/v2/' + domain + '/' + to;
        if (req.params.title) {
            uri = uri + '/' + req.params.title;
            if (req.params.revision) {
                uri = uri + '/' + req.params.revision;
            }
        } else if (from === 'html') {
            uri = uri + '/_'; // fake title to avoid <400/No title or wikitext was provided>
        }
        var request = {
            uri: uri,
    ...
    return {
        paths: {
            ...
            '/v1/{domain}/transform/html/to/html': {
                post: { request_handler: parsoidConvert(conf.parsoidHost, 'html', 'html') }
            },
            '/v1/{domain}/transform/html/to/wt': {
                post: { request_handler: parsoidConvert(conf.parsoidHost, 'html', 'wt') }
            },
            '/v1/{domain}/transform/wt/to/html': {
                post: { request_handler: parsoidConvert(conf.parsoidHost, 'wt', 'html') }
            }
        }
    };
};
