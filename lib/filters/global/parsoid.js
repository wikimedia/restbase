...
function getFormatRevision(parsoidHost, format) {
    return function (restbase, req) {
        ...
        if (isCacheMissForced(req)) {
            ...
            return revs.resolveMWOldid(restbase, domain, revision)
            .then(function (res) {
                return revs.updateRevTable(restbase, domain, bucket, key, revision, res.userId, res.userText, res.comment, tid);
            }).then(function (res) {
