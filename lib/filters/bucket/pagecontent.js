...
function checkResponse(restbase, req, res) {
    var rp = req.params;
        ...
        if (res.status === 404) {
            ...
            .then(function(parsoidResp) {
                ...
                if (parsoidResp.status === 200) {
                    ...
                    // XXX: Create a tid from the oldid's timestamp
...
PCBucket.prototype.getFormatRevision = function(restbase, req) {
    console.log('getFormatRevision');
    var rp = req.params;
    req.uri = '/v1/' + rp.domain + '/' + rp.bucket + '.' + rp.format + '/' +
        rp.key + '/' + rp.revision;
    return restbase.get(req)
    .then(checkResponse.bind(null, restbase, req));
};
