        ...
        if (res.status === 404 && /^[0-9]+$/.test(rp.revision)) {
            ...
            return restbase.get({
                ...
                if (parsoidResp.status === 200) {
                    ...
                    .catch(function(e) {
                        self.log('error/pagecontent/html/put', e);
                    });
                } else {
...
PCBucket.prototype.getFormatRevision = function(restbase, req) {
        ...
        .then(function(res) {
            if (res.status === 200) {
                ...
                req.uri = '/v1/' + rp.domain + '/' + rp.bucket + '.' + rp.format + '/' +
                    rp.key + '/' + tid;
                return restbase.get(req)
                .then(self.checkResponse.bind(self, restbase, req, tid, null));
            } else if (!/^[0-9]+$/.test(rp.revision)) {
                req.uri = '/v1/' + rp.domain + '/' + rp.bucket + '.' + rp.format + '/' +
                    rp.key + '/' + rp.revision;
                return restbase.get(req)
                .then(self.checkResponse.bind(null, restbase, req, null, null));
            } else {
    ...
    req.uri = '/v1/' + rp.domain + '/' + rp.bucket + '.' + rp.format + '/' +
        rp.key + '/' + rp.revision;
    return restbase.get(req)
    .then(self.checkResponse.bind(self, restbase, req, null, null));
};
