...
handler.prototype._assemble = function(spec) {
    var traverse = function(path) {
        // A simple dotted path reference
        var context = "req";
        path.split(/\./).forEach(function(pathBit) {
    ...
    if (spec === "$request") {
        ...
        for (var key in spec) {
            if (/^\$/.test(spec[key])) {
                code.push(traverse(spec[key].slice(1)));
            } else if (/^\{[^}]+\}$/.test(spec[key])) {
                var groups = /^\{([^}]+)\}$/.exec(spec[key]);
                code.push(traverse(groups[1]));
            } else if (/^\//.test(spec[key])) {
                // An URL template
                code.push("var urlTemplate = "+template.parse(spec[key])+";");
                code.push("newReq.push(urlTemplate.expand({request: req});");
            } else {
                // Treat it as a string literal
                code.push("newReq.push("+spec[key]+");");
            }
        }
    }
    return code.join('\n');
