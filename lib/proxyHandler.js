...
function parseVariable(variable, parentReq, res) {
	...
	if(variableSplit[0] === "$request") {
		...
		throw new Error('Error while parsing the variable' + variable);
	}
	...
	if (req === "$request") {
		...
		if (req.method) {
			if (isVariable(req.method)) {
				newReq.method = parseVariable(req.method, parentReq, res);
			} else {
			...
			if (/\{.*\}/.test(req.url)) {
				...
				for (var i=0; i<reqSplit.length-2; i++) {
					...
					if (arg && arg[1].constructor === String) {
						if (/\./.test(arg[1])) {
							newUrl[arg[1]] = parseVariable("$"+arg[1], parentReq, res);
				...
				if (/\{[a-z\.A-z]+\}[\{]?|[a-z\.A-Z]+\{/.test(reqSplit[reqSplit.length-2])) {
					...
					if (arg) {
						if (/\./.test(arg[1])) {
							...
							newUrl[arg[2]] = parseVariable("$"+arg[2], parentReq, res);
						} else if(parentReq.params[arg[1]]) {
		...
		if (req.headers) {
			if (isVariable(req.headers)) {
				newReq.headers = parseVariable(req.headers, parentReq, res);
			} else {
		...
		if (req.query) {
			for (var item in req.query) {
				if (isVariable(req.query[item])) {
					req.query[item] = parseVariable(req.query[item], parentReq, res);
				}
			}
		...
		if (req.body) {
			if (isVariable(req.body)) {
				newReq.body = parseVariable(req.body, parentReq, res);
			} else {
