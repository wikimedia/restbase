 ...
 * @param {string} [targetTid] the item tid.
 *                             Used in case item is a restriction to indicate the content tid.
 */
mwUtil.applyAccessChecks = function(item, targetTid) {
    // Page was deleted - new version of tracking
    if (item.page_deleted) {
        var deletionTime = uuid.fromString(item.page_deleted).getDate();
        var revisionTime = uuid.fromString(targetTid || item.tid).getDate();
