...
rbUtil._contentTypeCache = {};
rbUtil._parseContentType = function(typeString) {
    var type = rbUtil._contentTypeCache[typeString];
    if (!type) {
        type = contentType.parse(typeString);
        rbUtil._contentTypeCache[typeString] = type;
    }
    return type;
};

/**
 * Checks if the response content-type matches any of the
 * options in a provided list
 *
 * @param {Object} res response object to test
 * @param {Array} allowedTypes list of allowed content-type strings
 * @returns {boolean} whether the check succeed
 */
rbUtil.contentTypeMatch = function(res, allowedTypes) {
    var resContentTypeString = res.headers && res.headers['content-type'];
    if (resContentTypeString) {
        var resContentType = rbUtil._parseContentType(resContentTypeString);
        var expectedTypeVariants = allowedTypes.filter(function(typeString) {
            return typeString.indexOf(resContentType.type) === 0;
        })
        .map(rbUtil._parseContentType);
        if (expectedTypeVariants.length > 0) {
            return expectedTypeVariants.some(function(expectedType) {
                return !expectedType.parameters.profile
                    || expectedType.parameters.profile === resContentType.parameters.profile;
            });
        }
    }
    return false;
