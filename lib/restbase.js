...
function RESTBase (routers, options) {
    ...
    if (options.conf.monitoring && options.conf.monitoring.statsdHost) {
        this.timer = new util.Timer(options.conf.monitoring.statsdHost);
    }
...
RESTBase.prototype._request = function (req) {
    ...
    if (this.__depth > this.options.maxDepth) {
    ...
    if (parentReq && req.uri === parentReq.uri) {
        ...
        if (this.storageHandler) {
            ...
            var noCache = (parentReq.headers
                && parentReq.headers['cache-control']
                && (/no-cache/).test(parentReq.headers['cache-control']));
    ...
    for (; i < this._routers.length; i++) {
        ...
        if (match) {
            ...
            // start timer
            if (this.timer) {
                var statName = match.sortKey + "." + req.method;
                this.timer.start(statName);
            }
            ...
            if (handler && handler.request_handler) {
                ...
                .then(function(item){
                    if (self.timer) {
                        if (item.status >= 500) {
                            self.timer.count(statName);
                        }
                        self.timer.stop(statName);
                    }
                    return item;
                })
                .catch(function(err){
