...
function RESTBase(options, req) {
    if (options && options.constructor === RESTBase) {
        ...
        // Copy cookies to forward on first request
        if (this._recursionDepth === 1
            && req.headers
            && req.headers.cookie) {
            ...
            this.forwardedHeaders = options.forwardedHeaders;
        }
    } else {
        // Brand new instance
    ...
    if (handler) {
        ...
        if (match.permissions
            ...
            request = request.then(function(res) {
                var permissionCheckRestbase = self.makeChild(req);
                return auth.checkPermissions(permissionCheckRestbase, childReq, match.permissions)
                .then(function() {
                    return res;
                });
            });
