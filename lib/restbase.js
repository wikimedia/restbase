...
function RESTBase(options, req) {
    if (options && options.constructor === RESTBase) {
        ...
        this._accessRestrictions = par._accessRestrictions;
        this._internalReqWhitelist = par._internalReqWhitelist;
    } else {
        // Brand new instance
        ...
        this._internalReqWhitelist
            = constructInternalRequestRegex(options.conf.internal_request_whitelist);
    }
}

/**
 * Checks if the request URI was white-listed in the internal request config
 * @param req a request object to test
 * @returns {boolean} true if the request URI is white-listed as internal
 */
RESTBase.prototype.isInternalRequest = function(req) {
    if (this._internalReqWhitelist) {
        return this._internalReqWhitelist.test(req.uri.toString());
    }
    return false;
};
...
RESTBase.prototype.defaultWebRequestHandler = function(req) {
    ...
    if (Object.keys(this._accessRestrictions).length > 0 && this.isInternalRequest(req)) {
        rbUtil.copyForwardedCookies(this, req);
    }
...
RESTBase.prototype._request = function(req) {
    ...
    if (handler) {
        ...
        if (accessRestricted) {
            childRESTBase._accessRestrictions
                = auth.joinPermissions(match.permissions, this._accessRestrictions);
        }
        ...
        if (accessRestricted) {
            return P.all([
                reqHandlerPromise,
                auth.checkPermissions(childRESTBase, childReq)])
            .then(function(res) {
                return res[0];
            });
        } else {
