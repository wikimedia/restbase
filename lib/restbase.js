...
RESTBase.prototype.defaultWebRequestHandler = function(req) {
    ...
    if (this._accessRestrictions.length > 0 && this.isInternalRequest(req)) {
    ...
    if (handler) {
        ...
        if (accessRestricted) {
            childRESTBase._accessRestrictions
                = this._accessRestrictions.concat(match.permissions);
        }
        ...
        if (accessRestricted) {
            return P.all([
                reqHandlerPromise,
                auth.checkPermissions(childRESTBase, childReq, match.permissions)])
            .then(function(res) {
                return res[0];
            });
        } else {
