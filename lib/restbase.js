...
RESTBase.prototype._request = function (req) {
    ...
    var handler = match && match.methods && (match.methods[childReq.method] || match.methods.all);
    if (handler) {
        ...
        return Promise.try(handler(childRESTBase, childReq))
        .then(function(res){
            ...
            if (res.status >= 400 && !(res instanceof Error)) {
