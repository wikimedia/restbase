...
function RESTBase (options, req) {
    if (options && options.constructor === RESTBase) {
        ...
        this._priv.statsd = options.metrics;
    }
}
...
RESTBase.prototype._request = function (req) {
    ...
    if (handler) {
        ...
        // Call the handler with Promise.try to catch synchronous exceptions.
        ...
        .then(function(res){
            ...
            priv.statsd.endTiming(statName, [statusClass, 'ALL'], startTime);
        ...
        function(err){
            ...
            priv.statsd.timing(statName, [statusClass, 'ALL'],
                    Date.now() - startTime);
            throw err;
        });
    } else {
