 ...
 *
 * Current rules:
 *  - spec must have at least one path
 *  - each request handler spec must have 'on_request' property
 *  - returning steps can't be parallel
...
function validateSpec(spec) {
    ...
    Object.keys(spec.paths).forEach(function(path) {
        ...
        Object.keys(conf).forEach(function(method) {
            ...
            if (Object.keys(handlerConf.on_request[handlerConf.on_request.length - 1]).length > 1) {
